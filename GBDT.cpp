...
void GBDT_MODEL::load_Data_From_SQL() {
...
void GBDT_MODEL::load_Data_From_Tree(unordered_map<string, vector<string>> mData_Info) {

	return;
}

void GBDT_MODEL::choose_Feature(unordered_map<string, vector<string>> mData_Info, unordered_map<string, vector<string>>& data_Info_Left, unordered_map<string, vector<string>>& data_Info_Right,int& min_Feature_Index, string& min_Feature_Split) {
...
void GBDT_MODEL::data_Split(unordered_map<string, vector<string>> mData_Info, int field_Index, string field_Value, unordered_map<string, vector<string>>& data_Temp_Left, unordered_map<string, vector<string>>& data_Temp_Right) {
	bool temp = true;
...
double GBDT_MODEL::caculate_Square(unordered_map<string, vector<string>> data_Temp) {      	               //计算方差
	double sum = 0.0;
	int n = data_Info_Fields.size() - 1;
...
Tree_Node* GBDT_MODEL::build_Tree(Tree_Node* parent, Tree_Node* head, unordered_map<string, vector<string>> mData_Info) {         	//创建树
...
Tree_Node* GBDT_MODEL::build_Leaf_Node(Tree_Node* head, unordered_map<string, vector<string>> mData_Info) {
	//创建叶节点
	int n = data_Info_Fields.size() - 1;
...
void GBDT_MODEL::sub_Tree_Square(Tree_Node* head, double& square_Sum, int& leaf_Num) {
...
void GBDT_MODEL::copy_Node(Tree_Node* sub_Tree_Node, Tree_Node* head_Node) {
...
Tree_Node* GBDT_MODEL::copy_Tree(Tree_Node* head, Tree_Node* sub_Tree_Parent, unordered_map<int, vector<Tree_Node*>>& tree_Level_Temp) {
	if (!head)
		return nullptr;
...
void GBDT_MODEL::predict(unordered_map<string, vector<string>> mData_Info, Tree_Node* optimal) {  //只有一行数
	...
	if (data_Info_Types[optimal->node_Index] == "Category") {
		if (mData_Info[data_Info_Fields[optimal->node_Index]][0] < optimal->node_Value) {           //数值小于节点值，从左子树遍历
			if (optimal->left_Tree)
				...
				mData_Info["Volume"][0] = to_string(stof(mData_Info["Volume"][0]) + stof(optimal->node_Value)); return;}
		}else {
...
void GBDT_MODEL::residual_Caculation() {
	unordered_map<string, vector<string>> Data_Temp;
	int n = gradient_Boost_Trees.size() - 1;
	for (int i = 0; i < Data_Info["Hour"].size(); i++) {
		for (int j = 0; j < data_Info_Fields.size()-1; j++) {
			Data_Temp[data_Info_Fields[j]].emplace_back(Data_Info[data_Info_Fields[j]][i]);
		}
		predict(Data_Temp, gradient_Boost_Trees[n]);
		Data_Info["Volume"][i] = to_string(stof(Data_Info["Volume"][i]) - stof(Data_Temp["Volume"][0]));
	}
}

void GBDT_MODEL::gdbt_Model() {
	int i = 1;
	while (i <= 5) {                                           //TODO: 需加入其他的约束，总体误差小于某一阈值
		head = build_Tree(nullptr, nullptr, Data_Info);       //CART决策树的建立
		post_Pruning();                                      //决策树的剪枝
		gradient_Boost_Trees.emplace_back(head);            //每一轮的决策树
		residual_Caculation();                             //计算残差
		i++;
	}
	return;
}
...
int main(){
	...
	Data_Info.emplace("Volume", vector<string>{ "0.0"});

	GBDT_MODEL cart_test;
	//梯度提升树训练
	cart_test.gdbt_Model();
	//预测
	for (int i = 0; i < cart_test.gradient_Boost_Trees.size(); i++) {
		cart_test.predict(Data_Info, cart_test.gradient_Boost_Trees[i]);
	}
